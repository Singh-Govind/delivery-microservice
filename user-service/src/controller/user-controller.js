...
const BlacklistedToken = require("../model/blacklisted-token");
const RefreshToken = require("../model/refresh-token");

// helper functions
const { VALIDATION_ERROR, STATUS_BAD_REQUEST, NOT_FOUND_ERROR, UNAUTHORIZED_ERROR, FORBIDDEN_ERROR } = require("../constants/error-constants");
const { STATUS_OK, STATUS_CREATED, STATUS_ALREADY_EXISTS, STATUS_NOT_FOUND, STATUS_UNAUTHORIZED, STATUS_FORBIDDEN } = require("../constants/status-codes");
...
const login = async (req, res, next) => {
  ...
  try {
    ...
    let access_token = `Bearer ${generateToken(user)}`;
    let refresh_token = generateToken(user, "refresh");

    await RefreshToken.updateOne({ user_id: user.id }, { user_id: user.id, refresh_token }, { upsert: true });
    ...
    res.status(STATUS_OK).json({ msg: "Login successful", user, token: access_token, status: STATUS_OK });
  } catch (e) {
    next(e);
  }
};
...
const register = async (req, res, next) => {
  let { username, password, email, dob, first_name, last_name } = req.body;
  ...
  try {
    ...
    const userObj = {
      ...
      dob,
      first_name: makeStringLowercase(first_name || ""),
      last_name: makeStringLowercase(last_name || ""),
    };

    await User.create(userObj);

    res.status(STATUS_CREATED).json({ msg: "User registered", status: STATUS_CREATED });
  } catch (e) {
    next(e);
  }
};
...
const refreshToken = async (req, res, next) => {
  try {
    ...
    await RefreshToken.updateOne({ user_id: decoded.id }, { user_id: decoded.id, refresh_token });
    ...
    res.status(STATUS_OK).json({ msg: "token", token: access_token, status: STATUS_OK });
  } catch (e) {
    next(e);
  }
};
